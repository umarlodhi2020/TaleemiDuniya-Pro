...
async function deployToAlwaysdata() {
    ...
    try {
        ...
        const startCmd = `cd ${remoteDir} && killall -9 node 2>/dev/null; rm -rf auth_sessions && tar -xzf auth_sessions.tar.gz && export PATH=$PATH:$HOME/.npm-global/bin && (pm2 start server.js --name whatsapp-bot || pm2 restart whatsapp-bot || nohup node server.js > bot.log 2>&1 &)`;
