...
let content = fs.readFileSync('src/pages/super-admin/Subscriptions.jsx', 'utf-8');

// Add developerWhatsapp and banksList to initial state
const target_state = `  const [gatewaysConfig, setGatewaysConfig] = useState({
    stripe: {`;
const replacement_state = `  const [gatewaysConfig, setGatewaysConfig] = useState({
    developerWhatsapp: {
      enabled: true,
      number: '+923001234567',
    },
...
const target_bank_ui = `            {/* ----------------- BANK QR / RAAST CONFIG ----------------- */}
...
const replacement_bank_ui = `            {/* ----------------- MULTI-BANK CONFIG ----------------- */}
            <GlassCard className="p-7 border border-amber-500/30 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
              <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-5">
                <div className="flex items-center gap-3">
...
const target_grid = `          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">`;
const replacement_grid = `          <GlassCard className="p-6 mb-6 border border-green-500/30 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
...
fs.writeFileSync('src/pages/super-admin/Subscriptions.jsx', content, 'utf-8');
